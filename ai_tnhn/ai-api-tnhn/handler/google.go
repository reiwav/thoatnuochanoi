...
func (h *GoogleHandler) GenerateAIDynamicReport(c *gin.Context) {
	...
	g.Go(func() error {
		...
		for _, st := range summary.Stations {
			details = append(details, fmt.Sprintf("  - %s: Vận hành %d/%d bơm (Cập nhật lúc: %s)", st.Name, st.OperatingCount, st.PumpCount, st.LastUpdate))
		}
		pumpingSummaryStr = fmt.Sprintf("Tổng số trạm bơm: %d\nChi tiết vận hành:\n%s", len(summary.Stations), strings.Join(details, "\n"))
		return nil
	})
...
- Đề cập rõ tình hình vận hành các trạm bơm quan trọng (ví dụ: Yên Sở, Đồng Bông, v.v) dựa trên dữ liệu.
