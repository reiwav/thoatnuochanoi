...
func (s *service) GetStationsByType(ctx context.Context, stationType string) (interface{}, error) {
	...
	// Set a reasonable limit for listing stations
	// Note: BasicFilter implementation in this project might need manual limit setting if it supports it
	// Looking at fil.go, BasicFilter doesn't seem to have a SetLimit.
	// I'll just use it as is, or check if another filter implementation exists.
...
func (s *service) GetRainAnalytics(ctx context.Context, stationID int64, year int, month int, startDate, endDate string, groupBy string) (interface{}, error) {
	...
	// If groupBy is empty, default to "date"
	if groupBy == "" {
		if year > 0 && month == 0 {
			groupBy = "month" // Group by month if only year is provided to avoid token limit issues
		} else {
			groupBy = "date"
		}
	}

	// If year/month provided, override range
	if year > 0 {
		if month > 0 {
	...
	// Add formatted string values for better readability
	for i := range stats {
...
func (s *service) GetRainSummaryByWard(ctx context.Context, year int, month int, startDate, endDate string) (interface{}, error) {
	...
	// 1. Get all stations to map ID -> Ward
	...
	// 2. Get stats for ALL stations in range, grouped by station_id
	stats, err := s.waterSvc.GetRainAggregateStats(ctx, 0, startDate, endDate, "station_id")
	if err != nil {
		return nil, err
	}

	// 3. Re-aggregate by Ward name
	...
	for _, raw := range stats {
		// ... (keep robust extraction logic)
		// Robust number extraction from mongo results
		var stationID int64
