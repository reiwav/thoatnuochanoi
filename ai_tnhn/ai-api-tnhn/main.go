...
func main() {
	...
	authService := auth.NewService(tokenRepo, userRepo, roleRepo)
	...
	stationService := station.NewService(rainStationRepo, lakeStationRepo, riverStationRepo, orgRepo)
	...
	orgHandler := handler.NewOrganizationHandler(orgService, authService, roleRepo, contextWith)
	empHandler := handler.NewEmployeeHandler(empService, contextWith)
	stationHandler := handler.NewStationHandler(stationService, authService, contextWith)
