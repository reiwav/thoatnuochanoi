...
const PointCard = ({ point, expandedId, handleCardClick, updateLength, setUpdateLength, updateWidth, setUpdateWidth, updateDepth, setUpdateDepth, updateNote, setUpdateNote, updatePreviews, removeUpdateImage, resolveOnUpdate, setResolveOnUpdate, submitting, handleSubmitUpdate, handleUpdateImageChange, navigate, theme, report }) => {
...
const InundationDashboard = () => {
    ...
    return (
        <Box sx={{ px: 2, pt: 2 }}>
            ...
            <TextField
                fullWidth size="small" placeholder="Tìm kiếm theo tên đường, địa chỉ..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                sx={{ mb: 2, bgcolor: 'background.paper', borderRadius: 2, '& .MuiOutlinedInput-root': { borderRadius: 2 } }}
                InputProps={{
                    startAdornment: <IconSearch size={16} sx={{ color: 'text.disabled', mr: 1, fontSize: '1.1rem' }} />,
                    endAdornment: searchQuery ? (
                        <IconButton size="small" onClick={() => setSearchQuery('')}><IconX size={16} /></IconButton>
                    ) : null
                }}
            />
            <Stack spacing={1.5}>
                {loading ? [1, 2, 3].map(i => <Skeleton key={i} height={100} sx={{ borderRadius: 2 }} />) :
                    filteredPoints.length === 0 ? (
                        <Box sx={{ textAlign: 'center', py: 6 }}><Typography color="textSecondary" sx={{ fontStyle: 'italic' }}>Không có dữ liệu.</Typography></Box>
                    ) : filteredPoints.map(point => (
                        <PointCard
                            key={point.id}
                            point={point}
                            ...
                            setUpdateDepth={setUpdateDepth}
                            updateNote={updateNote}
                            setUpdateNote={setUpdateNote}
                            updatePreviews={updatePreviews}
                            removeUpdateImage={removeUpdateImage}
                            resolveOnUpdate={resolveOnUpdate}
                            setResolveOnUpdate={setResolveOnUpdate}
                            submitting={submitting}
                            handleSubmitUpdate={handleSubmitUpdate}
                            handleUpdateImageChange={handleUpdateImageChange}
                            navigate={navigate}
                            theme={theme}
                            report={point.active_report}
                        />
                    ))
                }
            </Stack>
