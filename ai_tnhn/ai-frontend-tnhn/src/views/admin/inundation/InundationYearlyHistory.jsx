...
import { IconFileExport, IconEye } from '@tabler/icons-react';
      ...
      <Stack spacing={3}>
        <Stack direction="row" spacing={2} alignItems="center" justifyContent="flex-start">
          <FormControl size="small" sx={{ minWidth: 120 }}>
            ...
            <Select value={year} label="Chọn năm" onChange={(e) => setYear(e.target.value)}>
              {years.map((y) => (
        ...
        <TableContainer
          component={Paper}
          sx={{ display: { xs: 'none', md: 'block' }, border: '1px solid', borderColor: 'divider', boxShadow: 'none', borderRadius: '4px' }}
        >
          <Table>
              ...
              {loading ? (
                ...
                aggregatedData.map((row, index) => (
                  <TableRow key={row.point_id || row.street_name || index} hover>
                    ...
                    <TableCell
                      align="center"
                      sx={{ borderRight: '1px solid #eee', fontWeight: 700, color: 'error.main', fontSize: '1.1rem' }}
                    >
                      {row.count}
                    </TableCell>
