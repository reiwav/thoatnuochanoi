...
export const getSlotTimestamp = (slotKey, selectedDate) => {
    ...
    if (slotKey === '6h30') {
        return targetDate.hour(6).minute(30).second(0).millisecond(0);
    }
    if (slotKey === '13h30') {
        return targetDate.hour(13).minute(30).second(0).millisecond(0);
    }
    if (slotKey === 'now') {
        const now = dayjs();
        return targetDate.hour(now.hour()).minute(now.minute()).second(now.second()).millisecond(0);
    }
    if (slotKey && slotKey.includes('_')) {
        const [offsetStr, timeStr] = slotKey.split('_');
        const dayOffset = parseInt(offsetStr, 10);
        const [h, m] = timeStr.split(':').map(Number);
        return targetDate.add(dayOffset, 'day').hour(h).minute(m).second(0).millisecond(0);
    }
    return targetDate;
};
