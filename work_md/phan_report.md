...
- **Kiểm tra:** Xác soát lại các hàm `AddUpdate situación`, `UpdateReport` và `ResolveInundation` trong `service.go` để đảm bảo lệnh `InundationReportRepo.Update` luôn được gọi khi có thay đổi tình hình.
